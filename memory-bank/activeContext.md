...
- **API Routes**: Built public API with movies, TV shows, and search endpoints. Added lazy-sync, pagination, and API-key validation with no application-side per-key or per-IP request limiter.
...
- **Usage Dashboard**: Added `/admin/usage` with 24h/7d/30d comparisons, active clients, success/cache rates, request charts, endpoints, countries, statuses, keys, P95 latency, upstream TMDB rate-limit reporting, filters, and pagination.
...
- **Rate Limits**: Removed all application-side request throttling. Upstream TMDB 429 responses retain `Retry-After` and are identified in usage logs.
- **MySQL Migrations**: Added a separate MySQL migration history and provider-aware migrations path. Existing production transitions once by preserving `User`/`ApiKey` and rebuilding disposable tables.
