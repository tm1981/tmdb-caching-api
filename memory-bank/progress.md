...
- [x] **API Routes**:
  ...
  - [x] Removed all application-side per-key and per-IP request throttling.
  - [x] Persist and display upstream TMDB rate limits separately from the app's IP guard.
  - [x] Added a separate MySQL migration history and one-time transition preserving only `User` and `ApiKey`.
- [x] **Admin Dashboard**:
