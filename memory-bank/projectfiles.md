...
## Database
- `prisma/schema.prisma`: Schema definition for PostgreSQL database (User, ApiKey, Movie, TvShow, SyncLog).
- `prisma/seed.ts`: Seed script to create admin user and default API key.

## Middleware
- `middleware.ts`: Protects `/admin/*` routes (session cookie check) and validates API keys on `/api/v1/*` routes.