...
const { countryCodeFromGeoIp, geoIpCountryCode } = await import('./geoip.ts')
...
assert.equal(normalizedRateLimitSource('tmdb'), 'tmdb')
assert.equal(normalizedRateLimitSource('tmdb-service'), 'tmdb-service')
assert.equal(normalizedRateLimitSource('unknown'), null)
...
assert.equal(await geoIpCountryCode('not-an-ip-address'), null)
assert.equal(countryCodeFromGeoIp({ registeredCountry: { isoCode: 'il' } }), 'IL')
assert.equal(countryCodeFromGeoIp({ country: { isoCode: 'US' }, registeredCountry: { isoCode: 'CA' } }), 'US')
