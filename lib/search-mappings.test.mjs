...
assert.equal(
  searchCaptureCacheKey(' No Match '),
  '/search/captured?query=no%20match',
)
assert.deepEqual(
  parseSearchCapture({
    query: 'No Match',
...
assert.equal(parseSearchCapture({ query: 'No Match', path: '/search/person' }), null)
assert.equal(parseSearchCapture({ query: 'No Match', path: '/search/movie', occurrences: 0 }), null)
